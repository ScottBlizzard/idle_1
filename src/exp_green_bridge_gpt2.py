...
def verify_v135_terminal_archive() -> dict:
    """Verify the immutable v1.3.5 engineering STOP before v1.3.6."""
    grandparent = verify_v134_terminal_archive()
    archive = PROJECT_ROOT / "analysis" / "GREEN_V135_TERMINAL_ARCHIVE_20260825"
    result_path = archive / "result.json"
    ledger_path = archive / "run_ledger.json"
    if not result_path.is_file() or sha256_file(result_path) != PREDECESSOR_RUN["result_sha256"]:
        raise GreenStop("00A_PREDECESSOR_ARCHIVE", "v1.3.5 result mismatch")
    if not ledger_path.is_file() or sha256_file(ledger_path) != "3659f737b36a81262bf21e1e47f598221f83550dde3bcb6e06f278779e0dfcaf":
        raise GreenStop("00A_PREDECESSOR_ARCHIVE", "v1.3.5 ledger mismatch")
    result = json.loads(result_path.read_text(encoding="utf-8"))
    ledger = json.loads(ledger_path.read_text(encoding="utf-8"))
    logs = sorted((archive / "development").glob("worker_*/worker.log"))
    required = {
        ...
        "confirmation": ledger.get("confirmation_started") is False,
        "eight_worker_logs": len(logs) == 8,
        "uniform_orientation_failure": all(
            "direct bypass and frame dimensions disagree" in path.read_text(encoding="utf-8")
            for path in logs
        ),
    }
    if not all(required.values()):
    ...
    return {
        ...
        "result_sha256": sha256_file(result_path),
        "ledger_sha256": sha256_file(ledger_path),
        "observed_terminal_condition": "direct-bypass matrix orientation mismatch",
        "grandparent": grandparent,
    }
...
def configure_runtime(device: str, physical_gpu: int = 4) -> dict:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical_gpu) or device != "cuda:0":
        raise GreenStop(
            "01_ENVIRONMENT",
            f"v1.3.6 requires physical GPU {physical_gpu} exposed as cuda:0",
        )
...
def verify_freeze(output_root: Path, require_confirmation: bool = False) -> dict:
    ...
    if manifest.get("schema_version") != "green-bridge-manifest-v1.3.6":
    ...
    if (
        ...
        raise GreenStop("17_MANIFEST_FREEZE", "v1.3.6 protocol identity changed")
    ...
    verify_v135_terminal_archive()
    invariance_path = output_root / "scientific_invariance_v136.json"
    ...
    for slot, (gate, frame) in enumerate(zip(SELECTED_GATES, design["gate_frames"])):
        ...
        if label == "active-identified":
            ...
            direct_common.append(direct_bypass_in_common_frame(
                values["rich"].D.T, frame, design["common"]
            ))
...
def _run_split_v136_multigpu(
    ...
    for worker_index, physical_gpu in enumerate(physical_gpus):
        ...
        contract = {
            "schema_version": "green-bridge-v1.3.6-worker-contract-v1",
    ...
    merge_payload = {
        "schema_version": "green-bridge-v1.3.6-multigpu-merge-v1",
        "split": split,
        "exact_batch_size": 1,
...
def _prepare_exact_batch_one_and_throughput_v136(
    model,
    tokens,
    suffix_ids,
    anchor,
    ...
    plan = {
        "schema_version": "green-bridge-hardware-batch-plan-v1.3.6",
    ...
    batch_payload = {
        "schema_version": "green-bridge-exact-batch-one-equivalence-v1.3.6",
        "exact_batch_size": 1,
    ...
    write_json_atomic(
        output_root / "manual_tail_batch_equivalence_v136.json",
        batch_payload,
    )
    ...
    throughput = {
        "schema_version": "green-bridge-tail-throughput-v1.3.6",
    ...
    write_json_atomic(output_root / "manual_tail_throughput_v136.json", throughput)
...
def _tail_preflight_v136(
    ...
    legacy_payload = {
        "schema_version": "green-bridge-tail-root-cause-v1.3.6",
    ...
    write_json_atomic(
        output_root / "manual_tail_root_cause_reproduction_v136.json",
        legacy_payload,
    )
    ...
    stage_payload = {
        "schema_version": "green-bridge-tail-stage-trace-v1.3.6",
    ...
    write_json_atomic(output_root / "manual_tail_stage_trace_v136.json", stage_payload)
    ...
    equivalence_payload = {
        "schema_version": "green-bridge-tail-equivalence-v1.3.6",
    ...
    write_json_atomic(output_root / "manual_tail_equivalence_v136.json", equivalence_payload)
    derivative_payload = {
        "schema_version": "green-bridge-tail-derivative-v1.3.6",
        "estimator": "central_finite_difference",
        "records": derivative_records,
        "near_zero_not_silently_dropped": True,
        "passed": True,
    }
    write_json_atomic(output_root / "manual_tail_derivative_v136.json", derivative_payload)
    ...
    target_payload = {
        "schema_version": "green-bridge-path-target-equivalence-v1.3.6",
        "mode": "joint",
        "subtract_residual_bypass": True,
        "code_isolated": True,
    ...
    write_json_atomic(output_root / "path_target_equivalence_v136.json", target_payload)
    hardware = _prepare_exact_batch_one_and_throughput_v136(
        model,
        clean_tokens,
        suffix_ids,
        anchor,
...
def prepare(output_root: Path, device: str) -> None:
    repository = assert_clean_repository()
    assert_empty_prepare_root(output_root)
    predecessor_archive = verify_v135_terminal_archive()
    environment = configure_runtime(device)
    from transformers import AutoTokenizer
    tokenizer = AutoTokenizer.from_pretrained(MODEL_ID, revision=MODEL_REVISION)
    pair_allowed = lambda first, second: token_pair_allowed(tokenizer, first, second)
    ...
    invariance = {
        "parent_schema": "green-bridge-v1.3.5",
        "current_schema": "green-bridge-v1.3.6",
        ...
        "allowed_differences": [
            ...
            "correct direct-bypass matrix orientation",
        ],
    }
    write_json_atomic(output_root / "scientific_invariance_v136.json", invariance)
    ...
    required = (
        ...
        "first_order_coefficients.npy", "scientific_invariance_v136.json",
        "manual_tail_root_cause_reproduction_v136.json",
        "manual_tail_stage_trace_v136.json",
        "manual_tail_equivalence_v136.json",
        "manual_tail_derivative_v136.json",
        "manual_tail_batch_equivalence_v136.json",
        "path_target_equivalence_v136.json", "hardware_batch_plan.json",
        "manual_tail_throughput_v136.json", "tail_audit.json",
        "prepare_result.json",
    )
    prepare_result = {
        "schema_version": "green-bridge-prepare-v1.3.6",
    ...
    manifest = {
        "schema_version": "green-bridge-manifest-v1.3.6",
        "execution_commit": repository["commit"],
        "review_commit": REVIEW_COMMIT,
        "repository": {
            "url": "https://github.com/ScottBlizzard/idle_1",
        ...
        "binding_decision": {
            "document": "analysis/CODEX_GREEN_V136_DIRECT_BYPASS_ORIENTATION_DECISION_20260825.md",
...
def development_phase(output_root: Path, device: str) -> None:
    ...
    gc.collect()
    torch_module().cuda.empty_cache()
    payload, dev_sd = _run_split_v136_multigpu(
...
def confirmation_phase(output_root: Path, device: str) -> None:
    ...
    gc.collect()
    torch_module().cuda.empty_cache()
    payload, _ = _run_split_v136_multigpu(
        confirmation,
        "confirmation",
        output_root,
        dev_noise,
    ...
    if total_survival < THRESHOLDS.total_cells_technical_min:
        ...
        result["first_failed_gate"] = None
    result["schema_version"] = "green-bridge-terminal-v1.3.6"
    write_json_atomic(output_root / "result.json", result)
