...
class ResourcePlan:
    ...
    conservative_mpfr_ops_per_cell: int
    minimum_cells: int
    minimum_mpfr_ops_per_row_radius: int
    frozen_mpfr_ops_cap_per_row: int
    feasible_under_frozen_cap: bool
    formula_version: str = "green-v400-gpt2-tail-resource-v1"
...
def plan_gpt2_tail_resources(shape: TailShape, *, mpfr_ops_per_term_lower: int = 12,
    ...
    if mpfr_ops_per_term_lower <= 0 or minimum_cells < 2 or frozen_mpfr_ops_cap_per_row <= 0:
