#!/usr/bin/env bash
# Frozen v1.3.3 launcher. Usage: bash src/launch_green_bridge_v133.sh GPU_ID PHASE
set -euo pipefail

GPU_ID="${1:-4}"
PHASE="${2:-prepare}"
if [[ "$GPU_ID" != "4" ]]; then
  echo "physical GPU must be 4 for v1.3.3" >&2
  exit 2
fi
...
RUNTIME_ROOT="/mnt/sdb/ccj/iclr_1_runs/green_bridge_v133_runtime"
mkdir -p \
...
TEST_LOG="/tmp/green_bridge_v133_contract_${PHASE}.log"
RUN_LOG="/tmp/green_bridge_v133_${PHASE}.log"
python src/test_green_bridge_contract.py \
  2>&1 | tee "$TEST_LOG"
python src/exp_green_bridge_gpt2.py --phase "$PHASE" --device cuda:0 \
  --output-root outputs/green_bridge_v133 2>&1 | tee "$RUN_LOG"
